...
def test_poll_indexes_written_traces(storage, index_state):
    ...
    rows = index_state.conn.execute(
        "SELECT trace_key, trace_state FROM traces ORDER BY uuidv7"
    ).fetchall()
    assert rows == [
        ("job-1/calc-1/started", TraceStates.OK.value),
        ("job-1/calc-1/finished", TraceStates.OK.value),
    ]
...
def test_poll_is_incremental(storage, index_state):
    ...
    write_trace(
        WriteTraceForm(
            ...
            author="bob",
            trace_state=TraceStates.WARNING,
            payload={},
        ),
        storage,
    )
    ...
    rows = index_state.conn.execute(
        "SELECT trace_key FROM traces ORDER BY uuidv7"
    ).fetchall()
