...
A key design goal: the storage part should be trivially implementable in any language.
For a simple trace it is just one S3 PUT; for a trace with blobs it is one PUT per blob plus one PUT for the record.
Avoid storage complexity that would make a Go or other-language port non-trivial.
