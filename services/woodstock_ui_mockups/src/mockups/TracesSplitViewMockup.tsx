...
// Minimum screen width to show split view (in pixels)
const MIN_SPLIT_VIEW_WIDTH = 1024;
...
export default function TracesSplitViewMockup() {
  ...
  const showSplitView = windowWidth >= MIN_SPLIT_VIEW_WIDTH;

  // Listen for window resize
  // Note: In real app, use react-resize-detector or similar for better performance
  useEffect(() => {
    ...
    >
      ...
      <div className="flex-1 flex overflow-hidden">
        ...
        <div
          ...
          style={{ width: showSplitView ? `${splitPosition}%` : "100%" }}
        >
          {/* Filter Sidebar */}
              ...
              <div className="bg-white rounded-lg shadow-sm border divide-y">
                {mockTraces.map((trace, index) => {
                  ...
                  const handleTraceClick = () => {
                    if (showSplitView) {
                      setSelectedTraceKey(trace.key);
                    } else {
                      // Below minimum width: open in new tab instead
                      window.open(
                        `/detail?trace=${encodeURIComponent(trace.key)}`,
                        "_blank"
                      );
                    }
                  };
        ...
        {/* Resizable Splitter - only shown in split view */}
        {showSplitView && (
          <div
        ...
        {/* Right Panel: Trace Detail - only shown in split view */}
        {showSplitView && (
          <div
              ...
              <div className="p-6 space-y-6">
                ...
                {Object.keys(selectedTrace.labels).length > 0 && (
                  <section>
                    ...
                    <div className="bg-white rounded-lg border shadow-sm p-4">
                      {Object.entries(selectedTrace.labels).map(
                        ([nodeKey, labels]) => (
                          <div key={nodeKey} className="mb-4 last:mb-0">
                            <h3 className="text-sm font-medium text-gray-700 mb-2 font-mono">
                              {nodeKey}
                            </h3>
                ...
                {/* Values */}
                {Object.entries(selectedTrace.payload).filter(
                  ([_, field]) => field.type === "value"
                ).length > 0 && (
                  <section>
                    <h2 className="text-lg font-semibold text-gray-900 mb-3">
                      Values
                ...
                {/* Links */}
                {Object.entries(selectedTrace.payload).filter(
                  ([_, field]) => field.type === "link" || field.type === "ref"
                ).length > 0 && (
                  <section>
                    <h2 className="text-lg font-semibold text-gray-900 mb-3">
                      Links
                    ...
                    <div className="bg-white rounded-lg border shadow-sm p-4 space-y-1">
                      {Object.entries(selectedTrace.payload)
                        .filter(
                          ([_, field]) =>
                            field.type === "link" || field.type === "ref"
                        )
                        .map(([key, field]) => (
                          <PayloadField
                            key={key}
                        ...
                        <>
                          ...
                          <div className="bg-white rounded-lg border shadow-sm overflow-hidden">
                            <div className="bg-gray-50 px-4 py-2 border-b flex items-center justify-between">
                              ...
                              <span className="text-xs text-gray-500">
                                tree://{selectedTrace.key}/config.json
                              </span>
                            </div>
                          ...
                          <div className="bg-white rounded-lg border shadow-sm overflow-hidden">
                            <div className="bg-gray-50 px-4 py-2 border-b flex items-center justify-between">
                              ...
                              <span className="text-xs text-gray-500">
                                tree://{selectedTrace.key}/results.md
                              </span>
                            </div>
                            <div className="p-4 prose prose-sm max-w-none">
                              <h4>Calculation Results</h4>
                              <p>
                                The calculation completed successfully with the
                                following metrics:
                              </p>
                              <ul>
                ...
                {Object.keys(selectedTrace.payload).length === 0 &&
                  Object.keys(selectedTrace.labels).length === 0 && (
